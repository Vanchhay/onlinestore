...
var OffCanvasSidebar = (function() {
	...
	function init() {
		...
		var container = $('#page-wrapper'),
			...
			sidebar = $('.sidebar-nav'),
			sidebarInner = $(sidebar).find('.sidebar-nav-inner'),
			top,
			winWidth,
			winHeight,
			...
			resetMenu = function() {
				...
				$('body').removeClass('offcanvas-sidebar-page')
				.css({
					'margin-left': '',
					'width': '',
					'height': ''
			...
			bodyClick = function(evt) {
				if( !hasParentClass( evt.target, 'sidebar-nav-inner' ) ) {
					resetMenu();
		...
		$(triggerBtn).bind(eventtype, function(evt){
			...
			$('body').addClass('offcanvas-sidebar-page')
			.css({
				'margin-left': '300px',
				'width': winWidth+'px',
				'height': winHeight+'px'
